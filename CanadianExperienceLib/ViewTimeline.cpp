            ...
            wxBORDER_SIMPLE)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewTimeline::OnStartFrame, this, XRCID("MachineStartFrame"));
...
/**
 * Machine>Set Start Frame menu handler (Must select machine with mouse first)
 * @param event
 */
void ViewTimeline::OnStartFrame(wxCommandEvent &event)
{
    TimelineStart dlg(this->GetParent(), GetPicture()->GetTimeline());
    if(dlg.ShowModal() == wxID_OK)
    {
