...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnChangeMachine, this,XRCID("MachineNumber"));
...
void ViewEdit::OnLeftDown(wxMouseEvent &event)
{
    ...
    if (hitActor != nullptr)
    {
        ...
        mLastSelectedDrawable = mSelectedDrawable;
    }
}
...
/**
 * Change the machine thats on
 * @param event
 */
void ViewEdit::OnChangeMachine(wxCommandEvent& event)
{
    mLastSelectedDrawable->MachDialog(this);
}
